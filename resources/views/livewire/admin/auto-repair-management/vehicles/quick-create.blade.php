<div class="p-6">
    @if($created)
        <div class="flex flex-col items-center text-center py-10">
            ...
            <p class="font-bold text-gray-900 dark:text-white">{{ __('auto-repair-management/vehicles.created') }}</p>
            @if($createdLabel)<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $createdLabel }}</p>@endif
            <button type="button" wire:click="addAnother" class="mt-6 text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">{{ __('auto-repair-management/vehicles.Add Vehicle') }}</button>
        </div>
    @else
...
<div><x-form.input name="vin" type="text" wire:model="vin" :label="__('auto-repair-management/vehicles.Vin')" class="dark:bg-gray-900" /></div></div>
        <div class="mt-8 flex justify-end"><x-button wire:click="store" variant="blue">{{ __('auto-repair-management/vehicles.Save') }}</x-button></div>
    @endif
</div>