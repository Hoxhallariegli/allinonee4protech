...
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->estimate_date?->format('d/m/Y H:i') ?? '-' }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->status }}</td>
    <td class="px-6 py-5 text-right !transition-none">
        <div class="flex justify-end gap-3 !transition-none">
            @can('edit_estimates')
                <x-a href="{{ route('admin.auto-repair-management.estimates.edit', $item) }}" class="!rounded-xl !bg-blue-50 dark:!bg-blue-900/30 !text-blue-600 dark:!text-blue-400 !px-4 !py-1.5 !text-[10px] !font-black !uppercase !border-none">Edit</x-a>
            @endcan
            @can('delete_estimates')
                <div x-data="{ confirmation: '' }" x-cloak class="inline-block">
                    <x-modal>
                        ...
                        <x-slot name="footer"><x-button variant="gray" @click="on = false">Cancel</x-button><x-button variant="red" x-bind:disabled="confirmation !== '{{ $item->id }}'" wire:click="$parent.deleteEstimate('{{ $item->id }}')" @click="on = false">Delete</x-button></x-slot>
                    </x-modal>
                </div>
            @endcan
        </div>
