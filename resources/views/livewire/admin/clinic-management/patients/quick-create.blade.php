<div class="p-6">
    @if($created)
        <div class="flex flex-col items-center text-center py-10">
            ...
            <p class="font-bold text-gray-900 dark:text-white">{{ __('clinic-management/patients.created') }}</p>
            @if($createdLabel)<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $createdLabel }}</p>@endif
            <button type="button" wire:click="addAnother" class="mt-6 text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">{{ __('clinic-management/patients.Add Patient') }}</button>
        </div>
    @else
