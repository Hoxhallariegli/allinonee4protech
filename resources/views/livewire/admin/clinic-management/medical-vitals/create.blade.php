<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('clinic-management/medical-vitals.Add MedicalVital') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('clinic-management/medical-vitals.New record') }}</x-short-description></div><x-back-btn route="admin.clinic-management.medical-vitals.index" /></div>
...
<div><x-form.input name="recorded_at" type="date" wire:model="recorded_at" :label="__('clinic-management/medical-vitals.Recorded At')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('clinic-management/medical-vitals.Save') }}</x-button></div></form></div>
</div>