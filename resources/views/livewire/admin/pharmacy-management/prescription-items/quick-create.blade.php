<div class="p-6">
    @if($created)
        <div class="flex flex-col items-center text-center py-10">
            ...
            <p class="font-bold text-gray-900 dark:text-white">{{ __('pharmacy-management/prescription-items.created') }}</p>
            @if($createdLabel)<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $createdLabel }}</p>@endif
            <button type="button" wire:click="addAnother" class="mt-6 text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">{{ __('pharmacy-management/prescription-items.Add PrescriptionItem') }}</button>
        </div>
    @else
...
<div><x-form.input name="quantity" type="text" wire:model="quantity" :label="__('pharmacy-management/prescription-items.Quantity')" class="dark:bg-gray-900" /></div></div>
        <div class="mt-8 flex justify-end"><x-button wire:click="store" variant="blue">{{ __('pharmacy-management/prescription-items.Save') }}</x-button></div>
    @endif
</div>