<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('travel-agency/flight-tickets.Add FlightTicket') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('travel-agency/flight-tickets.New record') }}</x-short-description></div><x-back-btn route="admin.travel-agency.flight-tickets.index" /></div>
...
<div><x-form.input name="price" type="text" wire:model="price" :label="__('travel-agency/flight-tickets.Price')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('travel-agency/flight-tickets.Save') }}</x-button></div></form></div>
</div>