<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('school-management/payments.Edit Payment') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('school-management/payments.Update info') }}</x-short-description></div><x-back-btn route="admin.school-management.payments.index" /></div>
...
<div><x-form.input name="payment_date" type="date" wire:model="payment_date" :label="__('school-management/payments.Payment Date')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('school-management/payments.Update') }}</x-button></div></form></div>
</div>