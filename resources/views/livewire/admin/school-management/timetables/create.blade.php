<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('school-management/timetables.Add Timetable') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('school-management/timetables.New record') }}</x-short-description></div><x-back-btn route="admin.school-management.timetables.index" /></div>
...
<div><x-form.input name="end_time" type="text" wire:model="end_time" :label="__('school-management/timetables.End Time')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('school-management/timetables.Save') }}</x-button></div></form></div>
</div>