<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('berber/bookings.Add Booking') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('berber/bookings.New record') }}</x-short-description></div><x-back-btn route="admin.berber.bookings.index" /></div>
    ...
    <div class="bg-white dark:bg-gray-800 p-8 sm:p-12 rounded-[2.5rem] shadow-sm border border-gray-50 dark:border-gray-700">
        <form wire:submit.prevent="store" class="space-y-8">
            ...
            <div class="mt-10 flex justify-end">
                <x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('berber/bookings.Save') }}</x-button>
            </div>
        </form>
    </div>
</div>
