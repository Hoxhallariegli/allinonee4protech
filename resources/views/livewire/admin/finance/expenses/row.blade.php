...
<td class="px-6 py-5 font-bold text-gray-900 dark:text-white">{{ $item->category?->name ?? '-' }}</td>
<td class="px-6 py-5">@if($item->attachment_file) <a href="{{ asset('uploads/'.$item->attachment_file) }}" target="_blank" rel="noopener"><x-heroicon-o-arrow-down-tray class="w-5 h-5 text-blue-500" /></a> @else - @endif</td>
    <td class="px-6 py-5 text-right !transition-none">
        <div class="flex justify-end gap-3 !transition-none">
            @can('edit_expenses')
                <x-a href="{{ route('admin.finance.expenses.edit', $item) }}" class="!rounded-xl !bg-blue-50 dark:!bg-blue-900/30 !text-blue-600 dark:!text-blue-400 !px-4 !py-1.5 !text-[10px] !font-black !uppercase !border-none">Edit</x-a>
            @endcan
            @can('delete_expenses')
                <div x-data="{ confirmation: '' }" x-cloak class="inline-block">
                    <x-modal>
                        ...
                        <x-slot name="footer"><x-button variant="gray" @click="on = false">Cancel</x-button><x-button variant="red" x-bind:disabled="confirmation !== '{{ $item->id }}'" wire:click="$parent.deleteExpense('{{ $item->id }}')" @click="on = false">Delete</x-button></x-slot>
                    </x-modal>
                </div>
            @endcan
        </div>
