<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('finance/transactions.Add Transaction') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('finance/transactions.New record') }}</x-short-description></div><x-back-btn route="admin.finance.transactions.index" /></div>
...
<div class="md:col-span-2"><x-form.textarea name="description" wire:model="description" :label="__('finance/transactions.Description')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('finance/transactions.Save') }}</x-button></div></form></div>
</div>