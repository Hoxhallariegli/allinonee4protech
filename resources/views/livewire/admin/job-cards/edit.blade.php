<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('job-cards.Edit JobCard') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('job-cards.Update info') }}</x-short-description></div><x-back-btn route="admin.job-cards.index" /></div>
...
<div><x-form.input name="closed_at" type="datetime-local" wire:model="closed_at" :label="__('job-cards.Closed At')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('job-cards.Update') }}</x-button></div></form></div>
</div>