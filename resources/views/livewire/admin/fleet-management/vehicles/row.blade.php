<tr class="hover:bg-gray-50/50 dark:hover:bg-gray-900/50 transition-none border-b border-gray-50 dark:border-gray-700/50 last:border-none">
    ...
    <td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->make }}</td>
<td class="px-6 py-5 text-gray-600 dark:text-gray-300">{{ $item->model }}</td>
...
<td class="px-6 py-5">@if($item->photo) <a href="{{ asset('uploads/'.$item->photo) }}" target="_blank" rel="noopener"><img src="{{ asset('uploads/'.$item->photo) }}" class="w-10 h-10 object-cover rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm"></a> @else - @endif</td>
    <td class="px-6 py-5 text-right !transition-none">
        <div class="flex justify-end gap-3 !transition-none">
            @can('edit_vehicles')
                <x-a href="{{ route('admin.fleet-management.vehicles.edit', $item) }}" class="!rounded-xl !bg-blue-50 dark:!bg-blue-900/30 !text-blue-600 dark:!text-blue-400 !px-4 !py-1.5 !text-[10px] !font-black !uppercase !border-none">Edit</x-a>
            @endcan
            @can('delete_vehicles')
