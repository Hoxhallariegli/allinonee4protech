<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('fleet-management/fuel-logs.Edit FuelLog') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('fleet-management/fuel-logs.Update info') }}</x-short-description></div><x-back-btn route="admin.fleet-management.fuel-logs.index" /></div>
...
<div><x-form.input name="cost" type="text" wire:model="cost" :label="__('fleet-management/fuel-logs.Cost')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('fleet-management/fuel-logs.Update') }}</x-button></div></form></div>
</div>