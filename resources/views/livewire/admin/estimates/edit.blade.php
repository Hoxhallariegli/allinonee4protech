<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('estimates.Edit Estimate') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('estimates.Update info') }}</x-short-description></div><x-back-btn route="admin.estimates.index" /></div>
...
<div><x-form.input name="status" type="text" wire:model="status" :label="__('estimates.Status')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('estimates.Update') }}</x-button></div></form></div>
</div>