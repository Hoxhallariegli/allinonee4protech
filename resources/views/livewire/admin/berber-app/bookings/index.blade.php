    ...
    <div class="card !p-0 overflow-hidden shadow-none border-gray-200 dark:border-gray-700 dark:bg-gray-800">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div><x-h1>{{ __('berber-app/bookings.Bookings') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('berber-app/bookings.List of') }} bookings</x-short-description></div>
...
<div><label class="block mb-1.5 text-[10px] font-bold uppercase tracking-widest ml-1 text-gray-900 dark:text-gray-100">Service Id</label><x-form.dropdown-search name="service_id" wire:model.live="service_id" label="none" :data="$services" placeholder="Filter Service Id" /></div>
        ...
        <div class="overflow-x-auto border-t border-gray-100 dark:border-gray-700">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <thead class="bg-gray-100/50 dark:bg-gray-700/50"><tr><x-table.th name="id" :label="__('berber-app/bookings.ID')" :$sortField :$sortAsc :sortable="true" /><x-table.th name="customer_id" :label="__('berber-app/bookings.Customer Id')" :$sortField :$sortAsc :sortable="in_array('customer_id', $sortableFields)" />
<x-table.th name="barber_id" :label="__('berber-app/bookings.Barber Id')" :$sortField :$sortAsc :sortable="in_array('barber_id', $sortableFields)" />
<x-table.th name="service_id" :label="__('berber-app/bookings.Service Id')" :$sortField :$sortAsc :sortable="in_array('service_id', $sortableFields)" />
<x-table.th name="appointment_datetime" :label="__('berber-app/bookings.Appointment Datetime')" :$sortField :$sortAsc :sortable="in_array('appointment_datetime', $sortableFields)" /><th class="px-6 py-4 text-right text-[10px] font-black uppercase text-gray-400 tracking-widest">{{ __('berber-app/bookings.Action') }}</th></tr></thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700/50">@forelse($items as $item) <livewire:admin.berber-app.bookings.row :$item :key="$item->id" /> @empty <tr><td colspan="100" class="px-6 py-10 text-center text-sm text-gray-400">{{ __('berber-app/bookings.No records found.') }}</td></tr> @endforelse</tbody>
            </table>
        </div>
