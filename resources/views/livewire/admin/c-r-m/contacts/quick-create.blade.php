<div class="p-6">
    @if($created)
        <div class="flex flex-col items-center text-center py-10">
            ...
            <p class="font-bold text-gray-900 dark:text-white">{{ __('c-r-m/contacts.created') }}</p>
            @if($createdLabel)<p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $createdLabel }}</p>@endif
            <button type="button" wire:click="addAnother" class="mt-6 text-xs font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">{{ __('c-r-m/contacts.Add Contact') }}</button>
        </div>
    @else
        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8"><div><x-form.input name="name" type="text" wire:model="name" :label="__('c-r-m/contacts.Name')" class="dark:bg-gray-900" /></div>
<div>
