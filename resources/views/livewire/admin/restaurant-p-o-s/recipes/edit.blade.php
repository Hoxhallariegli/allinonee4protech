<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('restaurant-p-o-s/recipes.Edit Recipe') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('restaurant-p-o-s/recipes.Update info') }}</x-short-description></div><x-back-btn route="admin.restaurant-p-o-s.recipes.index" /></div>
...
<div><x-form.input name="quantity_required" type="text" wire:model="quantity_required" :label="__('restaurant-p-o-s/recipes.Quantity Required')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('restaurant-p-o-s/recipes.Update') }}</x-button></div></form></div>
</div>