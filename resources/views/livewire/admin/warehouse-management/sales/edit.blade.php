<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('warehouse-management/sales.Edit Sale') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('warehouse-management/sales.Update info') }}</x-short-description></div><x-back-btn route="admin.warehouse-management.sales.index" /></div>
...
<div><x-form.input name="total" type="text" wire:model="total" :label="__('warehouse-management/sales.Total')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('warehouse-management/sales.Update') }}</x-button></div></form></div>
</div>