<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('warehouse-management/stock-adjustments.Add StockAdjustment') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('warehouse-management/stock-adjustments.New record') }}</x-short-description></div><x-back-btn route="admin.warehouse-management.stock-adjustments.index" /></div>
...
<div class="md:col-span-2"><x-form.textarea name="reason" wire:model="reason" :label="__('warehouse-management/stock-adjustments.Reason')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('warehouse-management/stock-adjustments.Save') }}</x-button></div></form></div>
</div>