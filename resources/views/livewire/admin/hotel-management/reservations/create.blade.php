<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('hotel-management/reservations.Add Reservation') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('hotel-management/reservations.New record') }}</x-short-description></div><x-back-btn route="admin.hotel-management.reservations.index" /></div>
...
<div><x-form.input name="total_price" type="text" wire:model="total_price" :label="__('hotel-management/reservations.Total Price')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('hotel-management/reservations.Save') }}</x-button></div></form></div>
</div>