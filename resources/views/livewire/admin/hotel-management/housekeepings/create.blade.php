<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('hotel-management/housekeepings.Add Housekeeping') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('hotel-management/housekeepings.New record') }}</x-short-description></div><x-back-btn route="admin.hotel-management.housekeepings.index" /></div>
...
<div><x-form.checkbox name="is_completed" wire:model="is_completed" :label="__('hotel-management/housekeepings.Is Completed')" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('hotel-management/housekeepings.Save') }}</x-button></div></form></div>
</div>