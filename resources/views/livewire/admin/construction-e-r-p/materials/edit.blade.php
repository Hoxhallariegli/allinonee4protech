...
<div><x-form.file-upload name="photo" wire:model="photo" :label="__('construction-e-r-p/materials.Photo')" id="photo" :isEditing="true" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('construction-e-r-p/materials.Update') }}</x-button></div></form></div>
</div>