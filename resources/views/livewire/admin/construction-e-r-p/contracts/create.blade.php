<div class="space-y-10">
    <div class="flex items-center justify-between gap-4 px-1"><div><x-h1>{{ __('construction-e-r-p/contracts.Add Contract') }}</x-h1><x-short-description class="dark:text-gray-400">{{ __('construction-e-r-p/contracts.New record') }}</x-short-description></div><x-back-btn route="admin.construction-e-r-p.contracts.index" /></div>
...
<div><x-form.input name="amount" type="text" wire:model="amount" :label="__('construction-e-r-p/contracts.Amount')" class="dark:bg-gray-900" /></div></div><div class="mt-10 flex justify-end"><x-button type="submit" variant="blue" class="w-full sm:w-auto !px-12 !py-4 !rounded-2xl">{{ __('construction-e-r-p/contracts.Save') }}</x-button></div></form></div>
</div>