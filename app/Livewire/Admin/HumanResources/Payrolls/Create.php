...
class Create extends Component
{
    ...
    public function render() {
        abort_if_cannot('add_payrolls');
        return view('livewire.admin.human-resources.payrolls.create', [
            'employees' => $this->getemployeesList(),
        ])->layout('components.layouts.app');
    }
    public function store(CreatePayrollAction $action) { $this->validate();  $dto = PayrollDTO::fromArray([
            'employee_id' => $this->employee_id,
            'month' => $this->month,
            'amount' => $this->amount,
            'is_paid' => $this->is_paid,
        ]); $action->execute($dto); session()->flash('success', __('human-resources/payrolls.created')); return to_route('admin.human-resources.payrolls.index'); }
