...
use App\Domain\WarehouseManagement\CustomerAddress\Queries\CustomerAddressListQuery;
use App\Domain\WarehouseManagement\CustomerAddress\Actions\DeleteCustomerAddressAction;
...
class CustomerAddresses extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.warehouse-management.customer-addresses.index', [
    ...
    public function deleteCustomerAddress($id, DeleteCustomerAddressAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('warehouse-management/customer-addresses.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('warehouse-management/customer-addresses.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('warehouse-management/customer-addresses.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('warehouse-management/customer-addresses.delete_error'), type: 'error'); }
    }
}