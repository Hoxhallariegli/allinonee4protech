...
use App\Domain\WarehouseManagement\PurchaseOrder\Queries\PurchaseOrderListQuery;
use App\Domain\WarehouseManagement\PurchaseOrder\Actions\DeletePurchaseOrderAction;
...
class PurchaseOrders extends Component
{
    ...
    public function deletePurchaseOrder($id, DeletePurchaseOrderAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('warehouse-management/purchase-orders.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('warehouse-management/purchase-orders.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('warehouse-management/purchase-orders.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('warehouse-management/purchase-orders.delete_error'), type: 'error'); }
    }
}