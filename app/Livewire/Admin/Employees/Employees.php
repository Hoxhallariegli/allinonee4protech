...
use App\Domain\Employee\Queries\EmployeeListQuery;
...
class Employees extends Component
{
    ...
    public function deleteEmployee($id, DeleteEmployeeAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('employees.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('employees.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('employees.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('employees.delete_error'), type: 'error'); }
    }
}