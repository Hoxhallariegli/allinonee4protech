...
use App\Domain\EventManagement\Booking\Actions\UpdateBookingAction;
...
class Edit extends Component
{
 ...
 public Booking $item;
    ...
    public function mount(Booking $booking) { $this->item = $booking; $this->fill($booking->toArray());  }
    public function render() {
        abort_if_cannot('edit_bookings');
        return view('livewire.admin.event-management.bookings.edit', [
    ...
    public function update(UpdateBookingAction $action) { $this->validate();  $dto = BookingDTO::fromArray([
            ...
            'status' => $this->status,
        ]); $action->execute($this->item, $dto); session()->flash('success', __('event-management/bookings.updated')); return to_route('admin.event-management.bookings.index'); }
