...
class Services extends Component
{
    ...
    public function deleteService($id, DeleteServiceAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('berber-app/services.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('berber-app/services.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('berber-app/services.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('berber-app/services.delete_error'), type: 'error'); }
    }
}