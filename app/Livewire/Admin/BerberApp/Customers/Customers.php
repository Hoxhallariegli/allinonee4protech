...
class Customers extends Component
{
    ...
    public function deleteCustomer($id, DeleteCustomerAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('berber-app/customers.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('berber-app/customers.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('berber-app/customers.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('berber-app/customers.delete_error'), type: 'error'); }
    }
}