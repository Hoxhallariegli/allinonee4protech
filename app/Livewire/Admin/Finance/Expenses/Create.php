...
class Create extends Component
{
    ...
    public function render() {
        abort_if_cannot('add_expenses');
        return view('livewire.admin.finance.expenses.create', [
            'categories' => $this->getcategoriesList(),
        ])->layout('components.layouts.app');
    }
    public function store(CreateExpenseAction $action) { $this->validate();         if ($this->attachment_file && !is_string($this->attachment_file)) { $this->attachment_file = $this->attachment_file->store('uploads/expenses', 'uploads'); }
 $dto = ExpenseDTO::fromArray([
            'amount' => $this->amount,
            'date' => $this->date,
            'category_id' => $this->category_id,
            'attachment_file' => $this->attachment_file,
        ]); $action->execute($dto); session()->flash('success', __('finance/expenses.created')); return to_route('admin.finance.expenses.index'); }
