...
use App\Domain\FacilityManagement\MaintenanceRequest\Actions\UpdateMaintenanceRequestAction;
...
class Edit extends Component
{
 ...
 public MaintenanceRequest $item;
    ...
    public function mount(MaintenanceRequest $maintenanceRequest) { $this->item = $maintenanceRequest; $this->fill($maintenanceRequest->toArray());  }
    public function render() {
        abort_if_cannot('edit_maintenance_requests');
        return view('livewire.admin.facility-management.maintenance-requests.edit', [
            'buildings' => $this->getbuildingsList(),
            'technicians' => $this->gettechniciansList(),
        ])->layout('components.layouts.app');
    }
    public function update(UpdateMaintenanceRequestAction $action) { $this->validate();  $dto = MaintenanceRequestDTO::fromArray([
            'building_id' => $this->building_id,
            'technician_id' => $this->technician_id,
            'description' => $this->description,
            'status' => $this->status,
        ]); $action->execute($this->item, $dto); session()->flash('success', __('facility-management/maintenance-requests.updated')); return to_route('admin.facility-management.maintenance-requests.index'); }
    protected function rules(): array { return MaintenanceRequest::rules($this->item->id); }
}