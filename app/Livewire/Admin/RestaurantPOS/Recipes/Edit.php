...
use App\Domain\RestaurantPOS\Recipe\Actions\UpdateRecipeAction;
...
class Edit extends Component
{
 ...
 public Recipe $item;
    ...
    public function mount(Recipe $recipe) { $this->item = $recipe; $this->fill($recipe->toArray());  }
    public function render() {
        abort_if_cannot('edit_recipes');
        return view('livewire.admin.restaurant-p-o-s.recipes.edit', [
            'menuItems' => $this->getmenuItemsList(),
            'ingredients' => $this->getingredientsList(),
        ])->layout('components.layouts.app');
    }
    public function update(UpdateRecipeAction $action) { $this->validate();  $dto = RecipeDTO::fromArray([
            'menu_item_id' => $this->menu_item_id,
            'ingredient_id' => $this->ingredient_id,
            'quantity_required' => $this->quantity_required,
        ]); $action->execute($this->item, $dto); session()->flash('success', __('restaurant-p-o-s/recipes.updated')); return to_route('admin.restaurant-p-o-s.recipes.index'); }
    protected function rules(): array { return Recipe::rules($this->item->id); }
}