...
use App\Domain\AutoRepairManagement\VehicleDocument\Queries\VehicleDocumentListQuery;
use App\Domain\AutoRepairManagement\VehicleDocument\Actions\DeleteVehicleDocumentAction;
...
class VehicleDocuments extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.auto-repair-management.vehicle-documents.index', [
    ...
    public function deleteVehicleDocument($id, DeleteVehicleDocumentAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-documents.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('auto-repair-management/vehicle-documents.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-documents.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-documents.delete_error'), type: 'error'); }
    }
}