...
use App\Domain\AutoRepairManagement\VehicleModel\Queries\VehicleModelListQuery;
use App\Domain\AutoRepairManagement\VehicleModel\Actions\DeleteVehicleModelAction;
...
class VehicleModels extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.auto-repair-management.vehicle-models.index', [
    ...
    public function deleteVehicleModel($id, DeleteVehicleModelAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-models.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('auto-repair-management/vehicle-models.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-models.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('auto-repair-management/vehicle-models.delete_error'), type: 'error'); }
    }
}