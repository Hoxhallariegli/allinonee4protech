...
use App\Domain\AutoRepairManagement\InvoiceItem\Queries\InvoiceItemListQuery;
use App\Domain\AutoRepairManagement\InvoiceItem\Actions\DeleteInvoiceItemAction;
...
class InvoiceItems extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.auto-repair-management.invoice-items.index', [
            'items' => $query->paginate($this->paginate),
            'sortableFields' => InvoiceItem::sortable(),
            'invoices' => \App\Models\AutoRepairManagement\Invoice::pluck('id', 'id')->toArray(),
    ...
    public function deleteInvoiceItem($id, DeleteInvoiceItemAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('auto-repair-management/invoice-items.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('auto-repair-management/invoice-items.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('auto-repair-management/invoice-items.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('auto-repair-management/invoice-items.delete_error'), type: 'error'); }
    }
}