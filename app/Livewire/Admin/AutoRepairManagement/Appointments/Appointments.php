...
use App\Domain\AutoRepairManagement\Appointment\Queries\AppointmentListQuery;
use App\Domain\AutoRepairManagement\Appointment\Actions\DeleteAppointmentAction;
...
class Appointments extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.auto-repair-management.appointments.index', [
    ...
    public function deleteAppointment($id, DeleteAppointmentAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('auto-repair-management/appointments.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('auto-repair-management/appointments.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('auto-repair-management/appointments.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('auto-repair-management/appointments.delete_error'), type: 'error'); }
    }
}