...
use App\Domain\AutoRepairManagement\JobCardPart\Actions\UpdateJobCardPartAction;
...
#[Title('Edit JobCardPart')]
class Edit extends Component
{
        use WithPagination;
 public JobCardPart $item;
    public $job_card_id = '';
    public $part_id = '';
    public $quantity = '';
    public $price = '';
    ...
    public function mount(JobCardPart $jobCardPart) { $this->item = $jobCardPart; $this->fill($jobCardPart->toArray());  }
    public function render() { abort_if_cannot('edit_job_card_parts'); return view('livewire.admin.auto-repair-management.job-card-parts.edit', [
            'jobCards' => $this->getjobCardsList(),
            'parts' => $this->getpartsList(),
        ])->layout('components.layouts.app'); }
    public function update(UpdateJobCardPartAction $action) { $this->validate();  $dto = JobCardPartDTO::fromArray([
            'job_card_id' => $this->job_card_id,
            'part_id' => $this->part_id,
            'quantity' => $this->quantity,
            'price' => $this->price,
        ]); $action->execute($this->item, $dto); session()->flash('success', __('auto-repair-management/job-card-parts.updated')); return to_route('admin.auto-repair-management.job-card-parts.index'); }
    protected function rules(): array { return JobCardPart::rules($this->item->id); }
}