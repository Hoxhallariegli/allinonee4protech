...
use App\Domain\AutoRepairManagement\PurchaseOrderItem\Queries\PurchaseOrderItemListQuery;
use App\Domain\AutoRepairManagement\PurchaseOrderItem\Actions\DeletePurchaseOrderItemAction;
...
class PurchaseOrderItems extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.auto-repair-management.purchase-order-items.index', [
            'items' => $query->paginate($this->paginate),
            'sortableFields' => PurchaseOrderItem::sortable(),
            'purchaseOrders' => \App\Models\AutoRepairManagement\PurchaseOrder::pluck('id', 'id')->toArray(),
    ...
    public function deletePurchaseOrderItem($id, DeletePurchaseOrderItemAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('auto-repair-management/purchase-order-items.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('auto-repair-management/purchase-order-items.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('auto-repair-management/purchase-order-items.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('auto-repair-management/purchase-order-items.delete_error'), type: 'error'); }
    }
}