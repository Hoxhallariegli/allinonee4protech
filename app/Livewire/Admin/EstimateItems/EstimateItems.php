...
use App\Domain\EstimateItem\Queries\EstimateItemListQuery;
use App\Domain\EstimateItem\Actions\DeleteEstimateItemAction;
...
class EstimateItems extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.estimate-items.index', [
            'items' => $query->paginate($this->paginate),
            'sortableFields' => EstimateItem::sortable(),
            'estimates' => \App\Models\Estimate::pluck('id', 'id')->toArray(),
    ...
    public function deleteEstimateItem($id, DeleteEstimateItemAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('estimate-items.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('estimate-items.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('estimate-items.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('estimate-items.delete_error'), type: 'error'); }
    }
}