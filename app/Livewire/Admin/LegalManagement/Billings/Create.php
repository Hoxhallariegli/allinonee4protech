...
class Create extends Component
{
    ...
    public function render() {
        abort_if_cannot('add_billings');
        return view('livewire.admin.legal-management.billings.create', [
            'cases' => $this->getcasesList(),
        ])->layout('components.layouts.app');
    }
    public function store(CreateBillingAction $action) { $this->validate();  $dto = BillingDTO::fromArray([
            'case_id' => $this->case_id,
            'amount' => $this->amount,
            'status' => $this->status,
        ]); $action->execute($dto); session()->flash('success', __('legal-management/billings.created')); return to_route('admin.legal-management.billings.index'); }
