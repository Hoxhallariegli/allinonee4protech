...
use App\Domain\JobCardService\Queries\JobCardServiceListQuery;
use App\Domain\JobCardService\Actions\DeleteJobCardServiceAction;
...
class JobCardServices extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.job-card-services.index', [
    ...
    public function deleteJobCardService($id, DeleteJobCardServiceAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('job-card-services.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('job-card-services.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('job-card-services.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('job-card-services.delete_error'), type: 'error'); }
    }
}