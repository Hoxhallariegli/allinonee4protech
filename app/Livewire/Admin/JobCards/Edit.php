...
use App\Domain\JobCard\Actions\UpdateJobCardAction;
...
class Edit extends Component
{
 ...
 public JobCard $item;
    ...
    {
        if (!$value) return;
        $related = \App\Models\Mechanic::find($value);
        if (!$related) return;
        if (isset($related->vehicle_id)) { $this->vehicle_id = $related->vehicle_id; }
        if (isset($related->customer_id)) { $this->customer_id = $related->customer_id; }
    }
    ...
    protected function getmechanicsList() {
        return \App\Models\Mechanic::with('employee')->get()->pluck('employee.name', 'id')->toArray();
    }
    ...
    public function render() { abort_if_cannot('edit_job_cards'); return view('livewire.admin.job-cards.edit', [
    ...
    public function update(UpdateJobCardAction $action) { $this->validate();  $dto = JobCardDTO::fromArray([
        ...
        ]); $action->execute($this->item, $dto); session()->flash('success', __('job-cards.updated')); return to_route('admin.job-cards.index'); }
