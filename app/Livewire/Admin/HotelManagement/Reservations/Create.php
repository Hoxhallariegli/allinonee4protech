...
class Create extends Component
{
    ...
    public function render() {
        abort_if_cannot('add_reservations');
        return view('livewire.admin.hotel-management.reservations.create', [
            'guests' => $this->getguestsList(),
            'rooms' => $this->getroomsList(),
        ])->layout('components.layouts.app');
    }
    public function store(CreateReservationAction $action) { $this->validate();  $dto = ReservationDTO::fromArray([
            'guest_id' => $this->guest_id,
            'room_id' => $this->room_id,
            'check_in' => $this->check_in,
            'check_out' => $this->check_out,
            'total_price' => $this->total_price,
        ]); $action->execute($dto); session()->flash('success', __('hotel-management/reservations.created')); return to_route('admin.hotel-management.reservations.index'); }
