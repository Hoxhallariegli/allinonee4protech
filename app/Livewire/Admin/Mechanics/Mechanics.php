...
use App\Domain\Mechanic\Queries\MechanicListQuery;
use App\Domain\Mechanic\Actions\DeleteMechanicAction;
...
class Mechanics extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.mechanics.index', [
            'items' => $query->paginate($this->paginate),
            'sortableFields' => Mechanic::sortable(),
            'employees' => \App\Models\Employee::pluck('name', 'id')->toArray(),
    ...
    public function deleteMechanic($id, DeleteMechanicAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('mechanics.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('mechanics.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('mechanics.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('mechanics.delete_error'), type: 'error'); }
    }
}