...
use App\Domain\PurchaseOrder\Actions\DeletePurchaseOrderAction;
...
class PurchaseOrders extends Component
{
    ...
    public function deletePurchaseOrder($id, DeletePurchaseOrderAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('purchase-orders.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('purchase-orders.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('purchase-orders.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('purchase-orders.delete_error'), type: 'error'); }
    }
}