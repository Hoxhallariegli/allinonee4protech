...
class Create extends Component
{
    ...
    public function render() {
        abort_if_cannot('add_assignments');
        return view('livewire.admin.school-management.assignments.create', [
            'schoolClasses' => $this->getschoolClassesList(),
            'subjects' => $this->getsubjectsList(),
        ])->layout('components.layouts.app');
    }
    public function store(CreateAssignmentAction $action) { $this->validate();  $dto = AssignmentDTO::fromArray([
            'school_class_id' => $this->school_class_id,
            'subject_id' => $this->subject_id,
            'title' => $this->title,
            'description' => $this->description,
            'due_date' => $this->due_date,
        ]); $action->execute($dto); session()->flash('success', __('school-management/assignments.created')); return to_route('admin.school-management.assignments.index'); }
