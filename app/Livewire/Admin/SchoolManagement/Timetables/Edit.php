...
use App\Domain\SchoolManagement\Timetable\Actions\UpdateTimetableAction;
...
class Edit extends Component
{
 ...
 public Timetable $item;
    ...
    public function mount(Timetable $timetable) { $this->item = $timetable; $this->fill($timetable->toArray());  }
    public function render() {
        abort_if_cannot('edit_timetables');
        return view('livewire.admin.school-management.timetables.edit', [
            'schoolClasses' => $this->getschoolClassesList(),
            'subjects' => $this->getsubjectsList(),
            'teachers' => $this->getteachersList(),
        ])->layout('components.layouts.app');
    }
    public function update(UpdateTimetableAction $action) { $this->validate();  $dto = TimetableDTO::fromArray([
        ...
        ]); $action->execute($this->item, $dto); session()->flash('success', __('school-management/timetables.updated')); return to_route('admin.school-management.timetables.index'); }
    protected function rules(): array { return Timetable::rules($this->item->id); }
}