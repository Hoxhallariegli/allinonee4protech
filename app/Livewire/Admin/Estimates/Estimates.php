...
use App\Domain\Estimate\Queries\EstimateListQuery;
use App\Domain\Estimate\Actions\DeleteEstimateAction;
...
class Estimates extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.estimates.index', [
    ...
    public function deleteEstimate($id, DeleteEstimateAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('estimates.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('estimates.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('estimates.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('estimates.delete_error'), type: 'error'); }
    }
}