...
use App\Domain\RealEstateCRM\Payment\Queries\PaymentListQuery;
use App\Domain\RealEstateCRM\Payment\Actions\DeletePaymentAction;
...
class Payments extends Component
{
    ...
    public function render()
    {
        ...
        return view('livewire.admin.real-estate-c-r-m.payments.index', [
    ...
    public function deletePayment($id, DeletePaymentAction $action) 
    {
        ...
        if (!$item) { $this->dispatch('toast', message: __('real-estate-c-r-m/payments.not_found'), type: 'error'); return; }
        try { $action->execute($item); $this->dispatch('toast', message: __('real-estate-c-r-m/payments.deleted'), type: 'success'); $this->resetPage(); } 
        catch (\Illuminate\Database\QueryException $e) { $this->dispatch('toast', message: __('real-estate-c-r-m/payments.delete_error_referenced'), type: 'error'); }
        catch (\Exception $e) { $this->dispatch('toast', message: __('real-estate-c-r-m/payments.delete_error'), type: 'error'); }
    }
}