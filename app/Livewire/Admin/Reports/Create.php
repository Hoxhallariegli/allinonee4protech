...
class Create extends Component
{
    ...
    public function render() { abort_if_cannot('add_reports'); return view('livewire.admin.reports.create', [
        ])->layout('components.layouts.app'); }
    public function store(CreateReportAction $action) { $this->validate();  $dto = ReportDTO::fromArray([
            'report_type' => $this->report_type,
            'report_date' => $this->report_date,
        ]); $action->execute($dto); session()->flash('success', __('reports.created')); return to_route('admin.reports.index'); }
